...
TEC_BIN=/home/administrador/concECPC/EPES-2.0/bin/modnlp-tecser-0.0.5-bin
TEC_SERVER=/usr/bin/java
ARGS="-Xms400m -Xmx450m -jar tecser.jar"
PIDFILE=/var/run/concECPCesser.pid
...
case "${1}" in
("start")
        echo "Stating concECPC ES server"
        ...
        ;;
("stop")
        echo "Stopping concECPC ES server"
        start-stop-daemon --stop --retry=1 --pidfile $PIDFILE 
#>/dev/null 2>&1
        exit $?
        ;;
(*)
        echo "Usage: /etc/init.d/concECPCes {start|stop}" >&2
        exit 3
        ;;
esac
