...
class RangeSliderUI extends BasicSliderUI {
  ...
  /** thumb shape (see RangeSlider for possible values) */
  private int thumbShapeType = RangeSlider.THUMB_PENTAGON;

  /** Color of selected range. */
  private Color rangeColor = new Color(166, 184, 204);
  private Color thumbColor = new Color(234, 242, 250);
  ...
  private Shape createThumbShape(int width, int height) {
    // Use circular shape.
    Shape shape;
    if (thumbShapeType == RangeSlider.THUMB_PENTAGON)
      shape = createPentagonalThumbShape(width, height);
    else if (thumbShapeType == RangeSlider.THUMB_CIRCLE)
      shape = new Ellipse2D.Double(0, 0, width, height);
    else // default to pentagon
      shape = createPentagonalThumbShape(width, height);
    return shape;
  }

  /**
   * Returns a polygon representing a thumb.
   */
  private Shape createPentagonalThumbShape(int width, int height) {
    // Use circular shape.
    int [] x = {0, width, width,    width/2, 0, 0};
    int [] y = {0, 0,     height/2, height, height/2, 0};
  ...
  protected void setThumbShapeType (int type) {
    switch (type) {
    case RangeSlider.THUMB_PENTAGON:
      thumbShapeType = RangeSlider.THUMB_PENTAGON;
      break;
    case RangeSlider.THUMB_CIRCLE:
      thumbShapeType = RangeSlider.THUMB_CIRCLE;
      break;
    default:
      thumbShapeType = RangeSlider.THUMB_PENTAGON;
      break;
